...
import {
  ...
  Platform, Alert, TextInput, Modal, KeyboardAvoidingView, ScrollView,
} from "react-native";
...
export default function CommunityScreen() {
  ...
  const handleLike = async (post: CommunityPost) => {
    if (likedPostIds.has(post.id)) return; // already liked
              ...
              <TouchableOpacity style={styles.card} onPress={() => handleOpenStory(item)} activeOpacity={0.75}>
                ...
                <View style={styles.cardContent}>
                  ...
                  <View style={styles.cardFooter}>
                    ...
                    <Text style={styles.cardDate}>{LANG_FLAGS[item.language] || "🌐"} {formatDate(item.createdAt)}</Text>
                  </View>
                  <View style={styles.cardStats}>
                    {item.likeCount > 0 && <Text style={styles.cardStat}>❤️ {item.likeCount}</Text>}
                    {item.downloadCount > 0 && <Text style={styles.cardStat}>⬇ {item.downloadCount}</Text>}
                  </View>
                </View>
